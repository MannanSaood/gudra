...
impl<T: Send> CudaGraph<T> {
    ...
    /// Execute a [`GraphNode`] on the graph's stream and synchronize it.
    ///
    ...
    /// # Ordering
    ///
    /// The operation is issued on [`stream`](CudaGraph::stream), and this
    /// method returns only after that stream completes. A later graph launch
    /// may therefore use any supported scheduling method.
    ///
    /// # Why the bounds
    ///
    /// The op must be a [`GraphNode`] (it neither allocates nor frees device
    /// memory) and must produce no output. Ops with an output, or ops that are not
        ...
        N: GraphNode + DeviceOp<Output = ()>,
    {
        ...
        let uncertain = crate::fault_policy::UncertainExecution::new("CUDA graph update");
        // SAFETY: the operation runs on this graph's retained stream.
        let result = unsafe { op.execute(&ctx) };
        if let Err(error) = unsafe { self.stream.synchronize() } {
            crate::fault_policy::fail_stop("CUDA graph update synchronization", error);
        }
