    ...
    Send + Sized + IntoFuture<Output = Result<<Self as DeviceOp>::Output, DeviceError>>
{
    ...
    fn sync_on(self, stream: &Arc<Stream>) -> Result<<Self as DeviceOp>::Output, DeviceError> {
        // Held until this function returns. An execute/synchronize unwind is
        // fail-stop because returning without proven completion could drop
        // owners while device work still uses them.
        let _execution_lock = acquire_execution_lock()?;
        let ctx = ExecutionContext::new(stream.clone());
        let uncertain = crate::fault_policy::UncertainExecution::new("blocking device operation");
        let res = unsafe { self.execute(&ctx) };
        if let Err(error) = unsafe { stream.synchronize() } {
            crate::fault_policy::fail_stop("blocking stream synchronization", error);
        }
