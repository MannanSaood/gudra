...
//! performed (a faulted context, a stream mid-capture) the worker aborts
//! before Rust destructors can release outer or backend-local owners. See
//! [`DeviceFuture`]'s type docs for why the wait is synchronous.
...
/// wait failure the worker aborts before any allocation owner can drop or be
/// reused.
///
...
pub struct DeviceFuture<T: Send, DO: DeviceOp<Output = T>> {
    ...
    /// Set before `execute` can submit work and cleared only after a callback,
    /// query, or synchronization proves completion.
    pub(crate) completion_pending: bool,
    /// Backend-local allocation owners destroyed during `execute` are moved
    /// here and released only after the stream completion terminal succeeds.
    pub(crate) quarantined_resources: Option<crate::device_buffer::QuarantinedResources>,
}
...
impl<T: Send, DO: DeviceOp<Output = T>> DeviceFuture<T, DO> {
    ...
    fn execute(&mut self) -> Result<(), DeviceError> {
        ...
        let uncertain = crate::fault_policy::UncertainExecution::new("asynchronous device submit");
        match unsafe { operation.execute(ctx) } {
            Ok(out) => {
                self.result = Some(out);
                self.quarantined_resources = Some(uncertain.handoff_to_terminal());
                Ok(())
            }
            Err(error) => {
                // Execution may have submitted a prefix before returning the
                // error. Drain it while backend-local device destructors are
                // quarantined, then return the recoverable execution error.
                if let Err(sync_error) = unsafe { ctx.get_cuda_stream().synchronize() } {
                    crate::fault_policy::fail_stop(
                        "asynchronous error-path synchronization",
                        sync_error,
                    );
                }
                self.completion_pending = false;
                uncertain.completion_proven();
                Err(error)
            }
        }
    }
    ...
    ///
    /// `completion_pending` is armed before [`execute`] because an operation
    /// can submit a prefix and then return an error without producing a
    /// result. It remains armed across callback registration and cancellation.
    ///
    /// [`execute`]: Self::execute
    fn has_unproven_submission(&self) -> bool {
        self.completion_pending
    }

    fn mark_completion_proven(&mut self) {
        self.completion_pending = false;
        if let Some(resources) = self.quarantined_resources.take() {
    ...
    /// the capture) and a faulted one cannot prove completion; both trigger
    /// the fail-stop policy in
    /// [`release_in_flight_result_with`](Self::release_in_flight_result_with).
    fn release_in_flight_result(&mut self) {
        if !self.has_unproven_submission() {
            return;
        }
        let stream = self
            .execution_context
    ...
    /// worker aborts because neither dropping nor leaking a borrowed child
    /// result retains every outer allocation owner.
        ...
        F: FnOnce() -> Result<(), DeviceError>,
    {
        ...
        if let Err(error) = wait() {
            crate::fault_policy::fail_stop("dropped asynchronous operation", error);
        }
        self.mark_completion_proven();
        ...
        match self.state {
            DeviceFutureState::Idle => {
                ...
                match spin_outcome {
                    ...
                    Ok(false) => {}
                    Err(e) => {
                        // The result stays stored; `Drop` fail-stops if the
                        // dead context cannot prove completion.
                        self.state = DeviceFutureState::Complete;
            ...
            DeviceFutureState::Executing => {
                ...
                match health {
                    StreamHealth::Faulted(e) => {
                        // The result stays stored for `Drop`, which fail-stops
                        // because the dead context cannot prove completion.
                        self.state = DeviceFutureState::Complete;
                        return Poll::Ready(Err(DeviceError::Driver(e)));
                    }
                    StreamHealth::Idle => {
...
mod release_tests {
    ...
    fn future_in_state<T: Send>(
        ...
        let completion_pending = matches!(state, DeviceFutureState::Executing)
            || (matches!(state, DeviceFutureState::Complete) && result.is_some());
        DeviceFuture {
            device_operation: None,
            execution_context: None,
            result,
    ...
    fn release_waits_before_dropping_the_result() {
        ...
        assert!(!future.has_unproven_submission());
    }

    /// Helper process for the failed-drain fail-stop assertion.
    #[test]
    fn failed_drain_helper() {
        if std::env::var_os("CUDA_ASYNC_FAILED_DRAIN_HELPER").is_some() {
    ...
    /// If a drain fails, the worker aborts before the result can drop.
    #[test]
    fn release_fail_stops_when_the_wait_fails() {
        let status = std::process::Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "device_future::release_tests::failed_drain_helper",
                "--nocapture",
            ])
            .env("CUDA_ASYNC_FAILED_DRAIN_HELPER", "1")
            .status()
            .expect("launch failed-drain helper");
        assert!(!status.success(), "failed drain unexpectedly returned");
        #[cfg(unix)]
        {
    ...
    /// A partial-submit error can have no result; it still must drain.
    #[test]
    fn release_drains_pending_submission_without_a_result() {
        let events = Arc::new(Mutex::new(Vec::new()));
        let mut future: DeviceFuture<u32, Value<u32>> =
            future_in_state(DeviceFutureState::Executing, None);
        future.release_in_flight_result_with(|| {
            events.lock().unwrap().push("wait");
            Ok(())
        });
        assert_eq!(events.lock().unwrap().as_slice(), ["wait"]);
        assert!(!future.completion_pending);
    }
