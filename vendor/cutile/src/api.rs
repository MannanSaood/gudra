...
impl<T: DType> DeviceOp for CopyDeviceToHostVec<T> {
    ...
    unsafe fn execute(
        ...
        // If the transfer errors or unwinds, this guard aborts before `host`
        // can be dropped. On success, ownership is returned to cuda-async's
        // terminal, which retains it until stream completion is proven.
        // SAFETY: `DeviceOp::execute` is reached through a cuda-async terminal,
        // which retains the returned host owner until it proves completion.
        let transfer_guard = unsafe {
            cuda_async::fault_policy::ExternalTransferGuard::new("device-to-host transfer")
        };
        if size > 0 {
            unsafe {
        ...
        transfer_guard.handoff_to_terminal();
        Ok(host)
    }
}
