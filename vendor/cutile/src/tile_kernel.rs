    ...
    DI: DeviceOp<Output = Tensor<T>>,
{
    ...
    unsafe fn execute(
        ...
        // SAFETY: this execute call is reached through a cuda-async terminal,
        // which retains the returned host owner until completion is proven.
        let transfer_guard = unsafe {
            cuda_async::fault_policy::ExternalTransferGuard::new("composed device-to-host transfer")
        };
        if size > 0 {
            unsafe {
                memcpy_dtoh_async(
