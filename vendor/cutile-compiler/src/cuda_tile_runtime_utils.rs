...
use uuid::Uuid;
#[path = "private_temp.rs"]
mod private_temp;
use private_temp::PrivateTemp;
...
fn probe_max_supported_bytecode_version(tileiras: &Path) -> Result<BytecodeVersion, JITError> {
    let private = PrivateTemp::new(&Uuid::new_v4().to_string()).map_err(|e| JITError::Generic(e.to_string()))?;
    let tmp_dir = private.path();
    ...
    for &version in BytecodeVersion::SUPPORTED.iter().rev() {
        ...
        private.write(bc_file.path(), &bytes).map_err(|e| {
            JITError::Generic(format!(
...
///
/// The temporary `.bc` and `.cubin` are removed before returning. Their
/// directory is also removed on compiler failure, launch failure, or unwind.
pub fn run_tileiras(
    bytecode: &[u8],
    gpu_name: &str,
    opts: &TileirasOptions,
    ...
    let private = PrivateTemp::new(&Uuid::new_v4().to_string()).map_err(|e| JITError::Generic(e.to_string()))?;
    let base_filename = private.path().join("kernel");
    ...
    private.write(bc_file.path(), bytecode).map_err(|e| {
    ...
    let output = match Command::new(&tileiras).args(&args).output() {
        Ok(output) => output,
        Err(e) => {
            // Diagnostics identify the invocation; intermediates are removed.
    ...
    if !output.status.success() {
        ...
        // Intermediates are removed even on compiler failure.
        return Err(JITError::Generic(format!(
            "{} failed while compiling Tile IR bytecode.\n\
             status: {}\n\
             command: {}\n\
             target gpu: {gpu_name}\n\
             bytecode: {bc_filename} (removed after compilation)\n\
    ...
    let cubin = private.read(cubin_file.path()).map_err(|e| {
        JITError::Generic(format!(
...
mod tests {
    ...
    #[test]
    #[cfg(target_os = "linux")]
    fn private_compilation_lifecycle() {
        use std::os::unix::fs::PermissionsExt;
        let _guard = ENV_LOCK.lock().unwrap();
        let fixture = PrivateTemp::new(&Uuid::new_v4().to_string()).unwrap();
        let compiler = fixture.path().join("compiler");
        let record = fixture.path().join("record");
        let _override = EnvVarGuard::set(TILEIRAS_PATH_ENV, &compiler);
        for (action, success) in [
            ("printf harmless > \"$out\"", true),
            ("exit 7", false),
            ("exit 0", false),
            (": > \"$out\"", false),
            ("ln -s /etc/passwd \"$out\"", false),
            ("ln \"$input\" \"$out\"", false),
        ] {
            fs::write(&compiler, format!("#!/bin/sh\nwhile [ $# -gt 0 ]; do\nif [ \"$1\" = -o ]; then shift; out=$1; fi\ninput=$1\nshift\ndone\nprintf '%s' \"$out\" > '{}'\n{}\n", record.display(), action)).unwrap();
            fs::set_permissions(&compiler, fs::Permissions::from_mode(0o700)).unwrap();
            let result = run_tileiras(b"fixture", "sm_120", &TileirasOptions::default());
            assert_eq!(result.is_ok(), success);
            let output = PathBuf::from(fs::read_to_string(&record).unwrap());
            assert!(!output.parent().unwrap().exists(), "directory survived compiler terminal");
        }
        // A real version probe traverses the same private-storage constructor.
        fs::write(&compiler, format!("#!/bin/sh\nfor arg in \"$@\"; do input=$arg; done\nprintf '%s' \"$input\" > '{}'\nexit 0\n", record.display())).unwrap();
        assert!(probe_max_supported_bytecode_version(&compiler).is_ok());
        let input = PathBuf::from(fs::read_to_string(&record).unwrap());
        assert!(!input.parent().unwrap().exists());
        fs::remove_file(&compiler).unwrap();
        assert!(run_tileiras(b"fixture", "sm_120", &TileirasOptions::default()).is_err());
    }
